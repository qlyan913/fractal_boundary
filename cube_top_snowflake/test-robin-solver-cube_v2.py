...
err=[]
err2=[]
PETSc.Sys.Print("Test: Solution u=2+x^3+y^2+3xy+yz on UnitCube")
...
NN=np.array([1.1*(mh[i]/mh[1])**3*err[1] for i in range(0,len(err))])
NN2=np.array([1.1*(mh[i]/mh[1])**2*err2[1] for i in range(0,len(err))])
plt.figure()
plt.loglog(mh, err,marker='o')
plt.loglog(mh, err2,marker='s')
plt.loglog(mh, NN)
plt.loglog(mh, NN2)
plt.legend(['$L^2$ error', '$H^1$ error', '$O(h^3)$','$O(h^2)$'])
plt.xlabel('maximum of mesh size')
plt.title('Test with solution: $u=2 + x^3 +y^2+3xy+y*z$')
plt.savefig("test_2.png")
PETSc.Sys.Print("Error vs mesh size  saved to test_2.png")
plt.close()

NN=np.array([(df[0]/df[i])**(4./3.)*err[0] for i in range(0,len(err))])
NN2=np.array([(df[0]/df[i])**(2./3.)*err2[0] for i in range(0,len(err))])
plt.figure()
plt.loglog(df, err,marker='o')
plt.loglog(df, err2,marker='s')
plt.loglog(df, NN)
plt.loglog(df, NN2)
plt.legend(['$L^2$ error', '$H^1$ error', '$O(dof^{-4/3})$','$O(dof^{-2/3})$'])
plt.xlabel('degree of freedom')
plt.title('Test with solution: $u=2 + x^3 +y^2+3xy+y*z$')
