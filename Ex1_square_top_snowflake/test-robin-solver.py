...
def snowsolver(mesh, D, Lambda, f, g, kl, kr, l,deg):
...
uh = snowsolver(mesh, D, Lambda, f, u, kl, kr, l,deg)
...
mesh_file = 'domain/trapezoid.msh'
mesh = Mesh(mesh_file)
x, y = SpatialCoordinate(mesh)
D = Constant(1.)
Lambda = Constant(1.)
f = Constant(0.)
u = interpolate(2 + x + 3*y, FunctionSpace(mesh, "Lagrange",deg))
kl = Constant(-1.)
kr = Constant(1.)
...
n = FacetNormal(mesh)
l2=inner(grad(u),n)+u
uh = snowsolver(mesh, D, Lambda, f, u, kl, kr, l2,deg)
...
plt.title('Test 3')
plt.savefig("figures/test_3.png")
...
plt.xlabel('maximum of mesh size')
plt.title('Test 4')
plt.savefig("figures/test_4.png")
print("Error vs mesh size  saved to figures/test_4.png")
plt.close()
...
plt.title('Test 4')
plt.savefig("figures/test_4_dof.png")
print("Error vs mesh size  saved to figures/test_4_dof.png")
